  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Active Deals', value: deals.filter((d) => !['closed_won','closed_lost'].includes(d.stage)).length, icon: GitBranch },
          { label: 'Total Value', value: `£${(totalValue / 1000).toFixed(0)}k`, icon: TrendingUp },
          { label: 'Unread Messages', value: Number(unreadMessages[0]?.count ?? 0), icon: MessageSquare },
          { label: 'Documents', value: Number(docCount[0]?.count ?? 0), icon: FileText },
