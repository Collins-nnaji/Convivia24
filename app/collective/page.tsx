  ...
  return (
    <div ref={ref} className="space-y-1">
      <div className="flex items-center justify-between">
        <span className={`text-[9px] uppercase tracking-widest ${dark ? 'text-zinc-500' : 'text-zinc-400'}`}>{label}</span>
        <span className={`text-[9px] font-black text-red-600`}>{pct}%</span>
      </div>
...
const services = [
  {
    ...
    bg: 'bg-zinc-900', textColor: 'text-white', descColor: 'text-zinc-400',
    accent: 'text-red-500', border: 'border-zinc-800',
    dark: true,
  },
  {
    num: '02',
    ...
    bg: 'bg-white', textColor: 'text-zinc-900', descColor: 'text-zinc-500',
...
function FaqItem({ q, a, idx }: { q: string; a: string; idx: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-sm font-bold text-zinc-900 group-hover:text-red-700 transition-colors">{q}</span>
        <ChevronDown
          size={16}
...
export default function WhatWeDoPage() {
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      {/* ── Hero ── */}
      <section className="pt-32 pb-16 px-6 bg-zinc-900 relative overflow-hidden">
        {/* background grid */}
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.88] text-white mb-6">
                What We<br />
                <span className="text-red-500 italic">Do.</span>
              </h1>
              ...
              <div className="flex gap-3">
                ...
                <Link href="/performance" className="inline-flex items-center gap-2 px-7 py-3.5 border border-zinc-700 text-zinc-300 text-sm font-semibold uppercase tracking-wider hover:border-zinc-400 hover:text-white transition-colors">
                  See Results
                </Link>
              </div>
            </motion.div>
              ...
              {[
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-300 mt-1">{s.label}</p>
        ...
        <section key={idx} className={`py-16 px-6 ${svc.bg} border-b ${svc.border}`}>
          <div className="max-w-6xl mx-auto">
            <div className={`grid lg:grid-cols-2 gap-12 items-start ${idx % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
              ...
              >
                <div>
                  <p className={`text-[10px] font-semibold uppercase tracking-widest ${svc.accent} mb-1`}>
                    {svc.num} — Service
                  </p>
                  <h2 className={`text-3xl md:text-4xl font-black tracking-tighter ${svc.textColor} leading-tight mb-1`}>
                    {svc.name}
                  </h2>
                  <p className={`text-sm italic ${svc.descColor}`}>{svc.tagline}</p>
                </div>

                <p className={`text-sm leading-relaxed ${svc.descColor}`}>{svc.description}</p>

                {/* Deliverables */}
                <div>
                  <p className={`text-[9px] font-semibold uppercase tracking-widest mb-3 ${svc.descColor}`}>Deliverables</p>
                  <ul className="space-y-2">
                    {svc.deliverables.map((d, dIdx) => (
                      <li key={dIdx} className={`flex items-center gap-3 text-sm ${svc.dark ? 'text-zinc-300' : 'text-zinc-600'}`}>
                        <div className="w-1 h-1 rounded-full bg-red-700 flex-shrink-0" />
                        {d}
                      </li>
                    ))}
                  </ul>
              ...
              >
                ...
                <div className={`border ${svc.border} p-6 space-y-5`}>
                  {/* SVG illustration */}
                  <div className={`w-full h-28 ${svc.dark ? 'bg-zinc-800' : 'bg-zinc-50'} flex items-center justify-center`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className={`p-4 border ${svc.dark ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50'}`}>
                      <p className={`text-2xl font-black leading-none ${svc.accent}`}>
                        {svc.metric}{svc.metricSuffix}
                      </p>
                      <p className={`text-[9px] uppercase tracking-widest mt-1 ${svc.descColor}`}>{svc.metricLabel}</p>
                    </div>
                    <div className={`p-4 border ${svc.dark ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50'}`}>
                      <p className={`text-2xl font-black leading-none ${svc.accent}`}>
                        <AnimatedCounter target={parseFloat(svc.metric2)} suffix={svc.metricSuffix2} />
                      </p>
                      <p className={`text-[9px] uppercase tracking-widest mt-1 ${svc.descColor}`}>{svc.metricLabel2}</p>
                    </div>
                  </div>

                  {/* Sparkline strip */}
                  <div className={`border-t pt-4 flex items-center justify-between ${svc.dark ? 'border-zinc-800' : 'border-zinc-100'}`}>
                    <p className={`text-[9px] uppercase tracking-widest ${svc.descColor}`}>Performance trend</p>
                    <Sparkline data={svc.spark} color={svc.dark ? '#ef4444' : '#b91c1c'} w={100} h={28} />
                  </div>
                </div>
              </motion.div>
      ...
      <section className="py-16 px-6 bg-zinc-900">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 text-center">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-red-500 mb-2">Process</p>
          ...
          <div className="grid md:grid-cols-3 gap-0 relative">
            {[
              ...
              >
                ...
                <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-600 mb-1">{step.duration}</p>
                <p className="text-5xl font-black text-zinc-800 leading-none mb-4 select-none">{step.step}</p>
                <h3 className="text-lg font-black text-white mb-3">{step.title}</h3>
                <p className="text-sm text-zinc-400 leading-relaxed mb-5">{step.desc}</p>
                <div className="border border-red-700/40 px-3 py-2 inline-block">
                  <p className="text-[9px] font-semibold uppercase tracking-widest text-red-500">Output</p>
                  <p className="text-xs font-bold text-white">{step.output}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-16 px-6 bg-white border-b border-zinc-100">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-red-700 mb-2">Engagement Models</p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 mb-4">Project-Based</p>
            ...
            >
              <div className="absolute top-4 right-4 bg-red-700 px-3 py-1">
                <p className="text-[9px] font-black uppercase tracking-widest text-white">Most Popular</p>
              </div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-red-500 mb-4">Retained Management</p>
              <h3 className="text-2xl font-black text-white mb-4">Full Managed Service</h3>
      ...
      <section className="py-16 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-red-700 mb-2">FAQ</p>
      ...
      <section className="bg-red-700 py-16 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-red-300 mb-2">Ready to start?</p>
