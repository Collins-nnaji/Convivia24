...
function toInvitee(i: Record<string, unknown>): CalendarInvitee {
  return {
    ...
    status: i.status as CalendarInvitee['status'],
    response_token: String(i.response_token),
  };
}
...
export interface InviteResponseInfo {
  invitee: CalendarInvitee;
  task: { id: string; title: string; starts_at: string; ends_at: string; location: string | null };
}
...
export async function listRange(userId: string, startIso: string, endIso: string): Promise<CalendarItem[]> {
  ...
  const invitees = taskIds.length
    ? await sql`SELECT id, task_id, name, email, status, response_token FROM personal_task_invitees WHERE task_id = ANY(${taskIds})`
    : [];
...
export async function getItem(userId: string, id: string): Promise<CalendarItem | null> {
  ...
  `;
  if (!row) return null;
  const invitees = await sql`SELECT id, name, email, status, response_token FROM personal_task_invitees WHERE task_id = ${id}`;
...
export async function createItem(userId: string, input: NewItemInput): Promise<CalendarItem> {
  ...
  for (const g of guests) {
    const [row] = await sql`
      ...
      RETURNING id, name, email, status, response_token
    `;
...
/** Looks up an invitee + their task by the invitee's public response token — the token itself is the credential, no user_id check. */
export async function getInviteByToken(token: string): Promise<InviteResponseInfo | null> {
  const [row] = await sql`
    SELECT i.id, i.name, i.email, i.status, i.response_token,
           t.id AS task_id, t.title, t.starts_at, t.ends_at, t.location
    FROM personal_task_invitees i
    JOIN personal_tasks t ON t.id = i.task_id
    WHERE i.response_token = ${token}
  `;
  if (!row) return null;
  return {
    invitee: toInvitee(row),
    task: {
      id: String(row.task_id),
      title: String(row.title),
      starts_at: new Date(row.starts_at as string).toISOString(),
      ends_at: new Date(row.ends_at as string).toISOString(),
      location: (row.location as string) ?? null,
    },
  };
}

export async function respondToInvite(token: string, status: 'accepted' | 'declined'): Promise<InviteResponseInfo | null> {
  const rows = await sql`
    UPDATE personal_task_invitees SET status = ${status} WHERE response_token = ${token} RETURNING id
  `;
  if (!rows.length) return null;
  return getInviteByToken(token);
}

/** Returns the user's ICS feed token, creating one on first request. */
export async function getOrCreateFeedToken(userId: string): Promise<string> {
  await sql`INSERT INTO calendar_feed_tokens (user_id) VALUES (${userId}) ON CONFLICT (user_id) DO NOTHING`;
  const [row] = await sql`SELECT token FROM calendar_feed_tokens WHERE user_id = ${userId}`;
  return String(row.token);
}

/** Resolves a feed token to its owner's upcoming items, for the public ICS subscribe endpoint. */
export async function listForFeedToken(token: string, daysAhead = 90, daysBehind = 7): Promise<CalendarItem[] | null> {
  const [row] = await sql`SELECT user_id FROM calendar_feed_tokens WHERE token = ${token}`;
  if (!row) return null;
  const start = new Date(); start.setDate(start.getDate() - daysBehind);
  const end = new Date(); end.setDate(end.getDate() + daysAhead);
  return listRange(String(row.user_id), start.toISOString(), end.toISOString());
}
