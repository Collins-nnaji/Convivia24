...
const RAW: NightEvent[] = [
  {
    id: 'lumen-thursdays',
    title: 'Lumen Thursdays',
    venueSlug: 'lumen-lounge',
    offsetDays: 0,
    startHour: 21,
    endHour: 2,
    tag: 'Rooftop',
    blurb: 'Resident selectors, bottle tables, VI skyline. Cardholders skip the lift queue.',
    expected: '~180 in',
    coverNgn: 15000,
  },
  {
    ...
    endHour: 5,
    tag: 'Club',
    blurb: 'Main floor until the lights come up. Drop bottles to the booth before midnight.',
    expected: '~320 in',
    coverNgn: 20000,
  },
  {
    id: 'terrace-golden',
    title: 'Golden Hour · Terrace 14',
    venueSlug: 'terrace-14',
    offsetDays: 0,
    startHour: 17,
    endHour: 22,
    tag: 'Lounge',
    blurb: 'Admiralty terrace from 5. Tequila tables, then the room gets louder.',
    expected: '~90 in',
    coverNgn: 10000,
  },
  {
    id: 'afterlight-gra',
    title: 'Afterlight GRA',
    venueSlug: 'afterlight',
    offsetDays: 2,
    startHour: 22,
    endHour: 4,
    tag: 'Club',
    blurb: 'Ikeja’s Saturday room. Mainland first, VI later if you still have legs.',
    expected: '~250 in',
    coverNgn: 8000,
  },
  {
    id: 'palm-supper',
    title: 'Palm Court Supper',
    venueSlug: 'palm-court',
    offsetDays: 1,
    startHour: 19,
    endHour: 23,
    tag: 'Dining',
    blurb: 'Ikoyi courtyard dinner that becomes Cognac. Cardholders: two-top before 8.',
    expected: '~60 seats',
  },
  {
    id: 'fifth-sunset',
    title: 'Sunset on Five',
    venueSlug: 'fifth-floor',
    offsetDays: 0,
    startHour: 16,
    endHour: 21,
    tag: 'Rooftop',
    blurb: 'Yaba golden hour. Canned cocktails, then whoever is still standing moves.',
    expected: '~110 in',
    coverNgn: 5000,
  },
  {
    id: 'saltwater-sunday',
    title: 'Saltwater Sundays',
    venueSlug: 'saltwater',
    offsetDays: 3,
    startHour: 13,
    endHour: 21,
    tag: 'Beach',
    blurb: 'Elegushi stretch, cabanas, Party Packs in the sand. Afterparty listed at 9.',
    expected: '~200 in',
    coverNgn: 12000,
  },
  {
    id: 'naija-live',
    title: 'Live at Naija House',
    venueSlug: 'naija-house',
    offsetDays: 2,
    startHour: 20,
    endHour: 1,
    tag: 'Live',
    blurb: 'Surulere live set. No velvet rope — just a room that sings back.',
    expected: '~140 in',
    coverNgn: 4000,
  },
  {
    id: 'copper-midweek',
    title: 'Copper Midweek',
    venueSlug: 'copper-bar',
    offsetDays: 4,
    startHour: 18,
    endHour: 23,
    tag: 'Whisky',
    blurb: 'Maryland whisky night. Soft booths, member-rate flights.',
    expected: '~40 in',
  },
  {
    id: 'lumen-sunday',
    ...
    endHour: 0,
    tag: 'Rooftop',
    blurb: 'Soft Sunday. Skyline, jazz into house, tables that stay.',
    expected: '~120 in',
    coverNgn: 10000,
  },
  {
    id: 'harbour-friday',
    title: 'Friday at Harbour',
    venueSlug: 'harbour-house',
    offsetDays: 5,
    startHour: 22,
    endHour: 4,
    tag: 'Club',
    blurb: 'The week’s first proper club night. Guest list for Resident cards after 11.',
    expected: '~280 in',
    coverNgn: 18000,
  },
  {
    ...
    endHour: 2,
    tag: 'Lounge',
    blurb: 'Lekki late lounge. Order the Tequila Terrace Pack to the booth.',
    expected: '~150 in',
    coverNgn: 12000,
  },
];
...
export function resolveEvent(raw: NightEvent): ResolvedEvent | null {
  const venue = getVenue(raw.venueSlug);
...
export function mergeEvents(extra: NightEvent[]): ResolvedEvent[] {
  const extraResolved = extra.map(resolveEvent).filter(Boolean) as ResolvedEvent[];
...
export function allEvents(): ResolvedEvent[] {
  return seedEvents();
}
...
export function eventsAtVenue(slug: string): ResolvedEvent[] {
  return allEvents().filter((e) => e.venueSlug === slug);
}
