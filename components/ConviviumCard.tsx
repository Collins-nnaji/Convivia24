...
/**
 * Access card — credit-card proportions (ISO/IEC 7810 ID-1).
 * loyalty: guest night card (red→black)
 * premium: partner Convivium Premium (obsidian + amber)
 * convivium: founding membership card
 */
export default function ConviviumCard({
  ...
  const brand = isLoyalty ? 'Guest Card' : isPremium ? 'The Convivium' : 'The Convivium';
  const sinceLabel = isLoyalty ? 'Points' : isPremium ? 'Premium pts' : 'Member Since';
  const sinceValue =
  ...
  const body =
    isLoyalty
      ? 'linear-gradient(150deg, #0a0a0a 0%, #2a0e0c 32%, #5c1814 68%, #8B2A22 100%)'
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          boxShadow:
            '0 30px 60px -18px rgba(0,0,0,0.65), 0 0 0 1px rgba(139,42,34,0.28), inset 0 1px 0 rgba(255,255,255,0.06)',
      ...
      >
        ...
        <div
          ...
          style={{
            background: isLoyalty
              ? 'linear-gradient(115deg, transparent 30%, rgba(255,255,255,0.08) 45%, rgba(163,56,46,0.35) 50%, rgba(255,255,255,0.08) 55%, transparent 70%)'
        ...
        <div
          ...
          style={{
            background: isLoyalty
              ? 'radial-gradient(ellipse 130% 90% at 15% 5%, rgba(163,56,46,0.28) 0%, transparent 55%)'
        ...
        <div
          ...
          style={{
            background: isLoyalty
              ? 'linear-gradient(90deg, transparent 0%, rgba(139,42,34,0.45) 20%, rgba(163,56,46,0.9) 50%, rgba(139,42,34,0.45) 80%, transparent 100%)'
        ...
        <div className="relative z-10 h-full flex flex-col justify-between p-6 sm:p-7">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center gap-2 tracking-[0.3em] text-cream/85 font-mono text-[13px] sm:text-[14px]">
              ...
              <span>••••</span>
              <span className="text-cream/90 tracking-widest">{cardNumber}</span>
            </div>
            <div className="flex items-end justify-between">
              <div>
                <p className="text-[8px] font-black uppercase tracking-[0.3em] text-cream/40 mb-0.5">
                  {isPremium ? 'Outlet' : 'Member'}
                </p>
                <p className="text-cream/95 text-[13px] font-semibold tracking-[0.15em]">{name}</p>
              </div>
              <div className="text-right">
                <p className="text-[8px] font-black uppercase tracking-[0.3em] text-cream/40 mb-0.5">{sinceLabel}</p>
                <p className="text-cream/80 text-[11px] font-medium tracking-wider">{sinceValue}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="absolute -z-10 -bottom-6 left-1/2 -translate-x-1/2 w-3/4 h-10 rounded-[50%] bg-ember/25 blur-2xl opacity-70" />
    </motion.div>
  );
}
